...
export default function AdminVerificationPage() {
  ...
  const filteredCompanies = useMemo(() => {
    return companies.filter((c) => {
      if (statusFilter === 'PENDING') return c.status !== 'REJECTED';
      if (statusFilter === 'REJECTED') return c.status === 'REJECTED';
      return true;
    });
  }, [companies, statusFilter]);

  // Paginated List for Grid View (10 per page)
  const paginatedGridCompanies = useMemo(() => {
    const start = (currentPage - 1) * 10;
    return filteredCompanies.slice(start, start + 10);
  ...
  const tableColumns: ColumnDef<CompanyVerificationItem>[] = [
    {
      key: 'no',
      header: 'No',
      align: 'center',
      className: 'w-12 text-center text-slate-500 dark:text-slate-400 font-medium',
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {c.hr_id_card_url ? (
            <a
              ...
              title="Lihat Berkas ID Card HRD"
            >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-3 flex-wrap">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all inline-flex items-center gap-2 ${
      ...
      {activeSearch && (
        <div className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 text-xs text-blue-900 dark:text-blue-300">
          <div className="flex items-center gap-2">
            ...
            <span>
              Hasil pencarian untuk: <strong className="font-bold underline">"{activeSearch}"</strong> ({filteredCompanies.length} data ditemukan)
            </span>
          </div>
          <button
            onClick={handleClearSearch}
      ...
      {/* Main Content Area */}
      {viewMode === 'table' ? (
        /* TABLE VIEW (Menggunakan Reusable DataTable Component dengan Pagination 10 Baris) */
        <DataTable<CompanyVerificationItem>
          ...
          emptyTitle={
            activeSearch
              ...
              : statusFilter === 'REJECTED'
              ? 'Tidak Ada Perusahaan Ditolak'
              : 'Tidak Ada Antrean Verifikasi'
          }
          emptyDescription={
            activeSearch
              ? `Tidak ada data perusahaan yang cocok dengan kata kunci "${activeSearch}".`
              : statusFilter === 'REJECTED'
              ? 'Belum ada pendaftaran perusahaan yang berada pada status penolakan.'
              : 'Semua pendaftaran perusahaan telah selesai diverifikasi atau belum ada pendaftaran baru.'
          }
        />
      ) : (
        /* GRID VIEW (Ringkas 2-3 Kolom dengan Pagination) */
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="p-12 text-center flex flex-col items-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="w-16 h-16 bg-blue-50 dark:bg-blue-950/40 text-blue-500 rounded-2xl flex items-center justify-center mb-4 border border-blue-100 dark:border-blue-800/60">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm max-w-sm mt-1.5">
                {activeSearch
                  ? `Tidak ada data perusahaan yang cocok dengan kata kunci "${activeSearch}".`
                  : statusFilter === 'REJECTED'
                  ? 'Belum ada pendaftaran perusahaan yang ditolak.'
                  : 'Semua pendaftaran perusahaan telah selesai diverifikasi.'}
              </p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {paginatedGridCompanies.map((c) => {
                  const isApproving = approvingId === c.id;
                  return (
                    <div
                      key={c.id}
