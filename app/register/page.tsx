...
export default function CompanyRegistrationFlow() {
  ...
  const handleStep1Submit = async (e: React.FormEvent) => {
    ...
    if (freeEmailDomains.includes(domain)) {
      setErrorStep1('Hanya email domain perusahaan yang diterima (contoh: hrd@tokopedia.com). Email pribadi tidak dapat digunakan.');
      return;
    }
  ...
  const handleStep3Submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!companyName.trim()) { setErrorStep3('Nama perusahaan wajib diisi.'); return; }
    if (!industri.trim()) { setErrorStep3('Sektor industri wajib dipilih.'); return; }
    if (!ukuran.trim()) { setErrorStep3('Jumlah karyawan wajib dipilih.'); return; }
    if (!nibNpwpNumber.trim() || !/^\d+$/.test(nibNpwpNumber)) { setErrorStep3('Nomor NIB / NPWP wajib diisi dan hanya boleh berupa angka.'); return; }
    if (!nibFile) { setErrorStep3('Dokumen NIB / NPWP wajib diunggah.'); return; }
    if (!companyAddress.trim()) { setErrorStep3('Alamat perusahaan wajib diisi.'); return; }
    if (!hrFullName.trim()) { setErrorStep3('Nama lengkap pendaftar wajib diisi.'); return; }
    if (!whatsappNumber.trim() || !/^\d+$/.test(whatsappNumber)) { setErrorStep3('Nomor WhatsApp wajib diisi dan hanya boleh berupa angka.'); return; }
    if (!hrPosition.trim()) { setErrorStep3('Jabatan pendaftar wajib diisi.'); return; }
    if (!idCardFile) { setErrorStep3('Foto ID Card / KTP pendaftar wajib diunggah.'); return; }
