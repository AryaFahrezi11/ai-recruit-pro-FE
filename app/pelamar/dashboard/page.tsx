...
export default function CandidateDashboardPage() {
  ...
  return (
    <div className="space-y-8 max-w-[1600px] w-full mx-auto">
      ...
      {/* Welcome Banner */}
      <div className="bg-[#0F766E] rounded-3xl p-8 sm:p-10 text-white shadow-md relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-3 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#E6FFFA] text-[#0F766E] text-xs font-bold shadow-2xs">
          ...
          <p className="text-[#E6FFFA]/90 text-sm sm:text-base leading-relaxed">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[#CCFBF1] shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-[#E6FFFA] border border-[#99F6E4] text-[#0F766E] flex items-center justify-center font-bold shrink-0">
            <FileText className="w-8 h-8 text-[#0F766E]" />
          </div>

          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-xs font-extrabold text-[#0F766E] bg-[#E6FFFA] px-3 py-0.5 rounded-full border border-[#99F6E4]">
        ...
        <Link
          ...
          className="px-6 py-2.5 rounded-full border-2 border-[#0F766E] text-[#0F766E] hover:bg-[#E6FFFA] text-xs sm:text-sm font-extrabold transition-all flex items-center gap-2 shrink-0"
        >
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between border-b border-[#CCFBF1] pb-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#E6FFFA] text-[#0F766E] text-xs font-extrabold mb-1">
              <Zap size={14} className="fill-current" /> Rekomendasi Otomatis Berbasis AI
            </div>
            <h2 className="text-2xl font-black text-[#0F766E]">
          ...
          <Link href="/pelamar/status" className="text-xs font-bold text-[#0F766E] hover:underline flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 gap-6">
          {matchedJobs.map((job) => {
            ...
            return (
              <div
                key={job.id}
                className="bg-white rounded-3xl border border-[#CCFBF1] p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 shadow-2xs hover:shadow-md transition-all"
              >
                <div className="flex items-start gap-5">
                  <img src={job.logo} alt={job.company} className="w-14 h-14 rounded-2xl object-cover border border-[#CCFBF1] shrink-0" />
                  <div className="space-y-2">
                    ...
                    <h3 className="text-xl font-bold text-[#0F766E]">{job.title}</h3>

                    <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 font-medium">
                      <span className="flex items-center gap-1"><MapPin size={14} /> {job.location} ({job.workType})</span>
                      <span>•</span>
                      <span className="flex items-center gap-1 text-[#0F766E] font-bold"><DollarSign size={14} /> {job.salary}</span>
                    </div>

                    <p className="text-xs text-slate-600 bg-[#F4FDFB] p-2.5 rounded-xl border border-[#CCFBF1] leading-relaxed">
                ...
                <div className="w-full lg:w-auto shrink-0 pt-2 lg:pt-0">
                  {isApplied ? (
                    ...
                    <button
                      ...
                      className="w-full lg:w-auto px-8 py-3.5 rounded-full bg-[#0F766E] hover:bg-[#0D635C] text-white font-extrabold text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2.5"
                    >
                      <Send size={16} />
