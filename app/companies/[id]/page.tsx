...
export default function GlintsCleanCompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchCompany = async () => {
      try {
        ...
        const [resComp, resAll] = await Promise.all([
          fetch(getApiUrl(`/perusahaan/${id}`)),
          fetch(getApiUrl('/perusahaan/verified'))
        ]);
        ...
        if (resComp.ok) {
          ...
          data.verified_date = data.verified_date || 'Terverifikasi Legalitas Mitra';
          data.jobs = Array.isArray(data.jobs) ? data.jobs : [];
          data.jobs_count = data.jobs_count || data.jobs.length;
          setCompany(data);

          if (resAll.ok) {
            const allComps = await resAll.json();
            if (Array.isArray(allComps)) {
              let similar = allComps.filter((c: any) => String(c.id) !== String(id) && c.industri === data.industri);
              if (similar.length === 0) {
                similar = allComps.filter((c: any) => String(c.id) !== String(id));
              }
  ...
  if (!company) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white dark:bg-slate-950 text-center px-4">
        <Building2 size={64} className="text-slate-300 dark:text-slate-700 mb-4" />
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">Profil Perusahaan Tidak Ditemukan</h2>
        <p className="text-slate-500 font-medium max-w-md">Profil perusahaan ini mungkin belum terverifikasi atau informasi tidak tersedia saat ini.</p>
        <button
          onClick={() => router.push('/companies')}
          className="px-6 py-3 rounded-full bg-[#1A4B9F] text-white font-bold hover:bg-[#133A7A] transition-colors mt-6"
        >
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans selection:bg-[#1A4B9F] selection:text-white transition-colors">
      ...
      {/* -------------------- GLINTS EXACT HEADER SECTION (CLEAN WHITE) -------------------- */}
      <section className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 py-10 px-6 sm:px-10 lg:px-16 transition-colors">
        <div className="max-w-[1300px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
              ...
              {/* Company Logo Box */}
              <div className="w-32 h-32 sm:w-36 sm:h-36 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-2.5 shadow-xs shrink-0 flex items-center justify-center overflow-hidden">
                {company.logo_url ? (
                  <img
              ...
              <div className="space-y-3 pt-1">
                ...
                {/* Location */}
                <div className="flex items-center justify-center sm:justify-start gap-2 text-sm sm:text-base text-slate-700 dark:text-slate-300 font-medium">
            ...
            <div className="space-y-3 text-center md:text-left border-t md:border-t-0 border-slate-100 dark:border-slate-800 pt-4 md:pt-1">
              
              {/* Employee Count */}
              <div className="flex items-center justify-center md:justify-start gap-2.5 text-sm sm:text-base text-slate-800 dark:text-slate-200 font-semibold">
                <Users size={18} className="text-slate-500 shrink-0" />
                <span>{company.ukuran || '50+ karyawan'}</span>
              </div>

              {/* Verified Status */}
              <div className="flex items-center justify-center md:justify-start gap-2 text-sm text-slate-700 dark:text-slate-300 font-medium">
                <ShieldCheck size={18} className="text-[#1A4B9F] shrink-0" />
      ...
      {/* -------------------- STICKY GLINTS SUB-TABS BAR -------------------- */}
      <div className="sticky top-20 z-40 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-2xs transition-colors">
        <div className="max-w-[1300px] mx-auto px-6 sm:px-10 lg:px-16 flex items-center gap-6 sm:gap-8 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {[
            ...
            <button
              ...
              className={`py-4 text-sm font-semibold whitespace-nowrap transition-colors relative cursor-pointer ${
                activeTab === tab.id
      ...
      <main className="max-w-[1300px] mx-auto px-6 sm:px-10 lg:px-16 py-10 space-y-10 flex-1 w-full">
                    ...
                    <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-400">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors ${
                          isSaved ? 'text-[#1A4B9F] fill-current' : 'text-slate-400'
                        }`}
                      >
